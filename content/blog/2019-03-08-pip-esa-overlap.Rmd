...
p_overlap_age <- ggplot(overlap_age2 %>% filter(age != "Total"),
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        legend.position = "bottom", legend.title = element_blank()) +
  labs(x = "Quarter", y = "Percent of claimants",
       title = "Percentage of one group of benefit recipients receiving the other type of benefit",
       subtitle = "5 Year Age Bands",
       caption = "(c) Evan Odell | 2019 | CC-BY-SA") +
  guides(colour=guide_legend(nrow=2, byrow=TRUE)) + 
  facet_wrap(~age)
...
overlap_age_all2 <- overlap_age_all %>% spread(key = grouped, value = value)

names(overlap_age_all2) <- snakecase::to_snake_case(names(overlap_age_all2))

overlap_age_all2 <- overlap_age_all2 %>%
  ...
  mutate(type = recode(type, 
         ...
         type = factor(type, levels=c("Claiming PIP/DLA/AA only", "Claiming both types of benefits", "Claiming INCAP only")))
```



```{r plot3}
p_overlap_count <- ggplot(data = overlap_age_all2 %>% filter(age == "Total"), 
                           aes(x=date, y = count)) + 
  ...
  labs(x = "Quarter", y = "Number of claimants",
       title = "Total of one group of benefit recipients receiving the other type of benefit",
...
```{r plot4}
p_overlap_count_age <- ggplot(data = overlap_age_all2 %>% filter(age != "Total"),
                           aes(x=date, y = count)) + 
  geom_area(aes(fill= type), position = 'stack') + 
  scale_fill_viridis_d() + 
  scale_y_continuous(labels = scales::comma) +
